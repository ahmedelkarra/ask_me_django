...
export default function AddAnswerDialog() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (valueInput.body) {
            try {
                const data = (await axios.post(`/api/quisition/comment/${param?.id}/`, valueInput, { headers: { "X-CSRFToken": token } })).data as { message: string }
    ...
    return (
        <React.Fragment>
            ...
            <Button variant="outlined" color="success" sx={{ margin: "10px 0" }} onClick={handleClick}>Add Answer</Button>
                    ...
                    <DialogContent>
                        <DialogContentText id="alert-dialog-slide-description" component={'div'}>
                            <Grid container spacing={'10px'}>
                                ...
                                <Grid item xs={12}>
                                    <TextField multiline rows={3} variant='standard' label="Enter your answer" type='text' fullWidth inputProps={{ maxLength: 50 }} onChange={(e) => setValueInput({ ...valueInput, body: e.target.value })} value={valueInput.body} required />
                                </Grid>
                            </Grid>
                        </DialogContentText>
                    </DialogContent>
